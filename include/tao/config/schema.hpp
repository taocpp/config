...
{
   const value& operator/( const value& v, const std::string& k )
   {
   ...
   namespace schema
   {
      ...
      struct node : all_of
      {
         ...
         {
            // description
            if( const auto& n = v / "description" ) {
               m_description = n.as< std::string >();
            }

            // sub/ref
            add< ref >( v / "type", m, path );

            // structural
            add< not_ref >( v / "not", m, path );

            add< list< all_of, ref > >( v / "all_of", m, path );
            add< list< any_of, ref > >( v / "any_of", m, path );
            add< list< one_of, ref > >( v / "one_of", m, path );

            if( const auto& n = v / "if" ) {
               auto p = std::make_unique< if_then_else >( n, m, path );
               if( const auto& t = v / "then" ) {
                  p->m_then = std::make_unique< ref >( t, m, path );
               }
               if( const auto& e = v / "else" ) {
            ...
            // value (generic)
            add< constant >( v / "const" );
            add< list< any_of, constant > >( v / "enum" );

            // string/binary
            add< length >( v / "length" );
            add< min_length >( v / "min_length" );
            add< max_length >( v / "max_length" );

            // string
            add< pattern >( v / "pattern" );

            // binary
            // TODO

            // number
            add< minimum >( v / "minimum" );
            add< maximum >( v / "maximum" );
            add< exclusive_minimum >( v / "exclusive_minimum" );
            add< exclusive_maximum >( v / "exclusive_maximum" );
            add< multiple_of >( v / "multiple_of" );

            // array
            add< min_items >( v / "min_items" );
            add< max_items >( v / "max_items" );
            add< array_ref >( v / "items", m, path );
            if( const auto& n = v / "unique_items" ) {
               if( n.as< bool >() == true ) {
            ...
            // object
            add< min_properties >( v / "min_properties" );
            add< max_properties >( v / "max_properties" );
            add< keys_ref >( v / "property_names", m, path );
            if( const auto& n = v / "properties" ) {
               auto p = std::make_unique< properties >( n, m, path );
               if( const auto& n = v / "additional_properties" ) {
                  p->m_default = std::make_unique< ref >( n, m, path );
               }
               m_properties.emplace_back( std::move( p ) );
            }
            else {
               add< object_ref >( v / "additional_properties", m, path );
            }
            add< list< all_of, has_property > >( v / "required" );

            // define
            if( const auto& d = v / "definitions" ) {
               for( const auto& e : d.get_object() ) {
                  ...
                  if( !m.emplace( p, std::move( n ) ).second ) {
                     ...
                     ( d / e.first ).append_message_extension( os );
                     throw std::runtime_error( os.str() );
                  }
               }
            }
      ...
      struct validator
      {
         ...
         {
            m_nodes.emplace( "null", std::make_unique< is_null_node >() );
            m_nodes.emplace( "boolean", std::make_unique< is_boolean_node >() );
            m_nodes.emplace( "number", std::make_unique< is_number_node >() );
            m_nodes.emplace( "string", std::make_unique< is_string_node >() );
            m_nodes.emplace( "binary", std::make_unique< is_binary_node >() );
            m_nodes.emplace( "array", std::make_unique< is_array_node >() );
            m_nodes.emplace( "object", std::make_unique< is_object_node >() );

            m_nodes.emplace( "integer", std::make_unique< is_integer_node >() );

            m_nodes.emplace( "", std::make_unique< node >( v, m_nodes ) );
