...
namespace tao::config::internal
{
   ...
   inline std::size_t erase_star( const position& pos, concat& l )
   {
      ...
      for( auto& i : l.private_entries() ) {
         ...
         throw pegtl::parse_error( format( __FUNCTION__, "attempt to delete everything from non-container", { { "non-container", { &i.position(), i.type() } } } ), pos );
      }
      return r;
   }
   ...
   inline std::size_t erase_minus( const position& pos, concat& l )
   {
      for( auto& i : reverse( l.private_entries() ) ) {
         if( !i.is_array() ) {
            throw pegtl::parse_error( format( __FUNCTION__, "attempt to delete last element from non-array", { { "non-array", { &i.position(), i.type() } } } ), pos );
         }
         auto& a = i.get_array();
      ...
      throw pegtl::parse_error( format( __FUNCTION__, "array has no last element to delete in", { { "array", { &l.p } } } ), pos );
   }
   ...
   inline std::size_t erase_star( const position& pos, concat& l, const key& p, const part& f )
   {
      ...
      for( auto& i : l.private_entries() ) {
         ...
         throw pegtl::parse_error( format( __FUNCTION__, "attempt to delete in non-container", { { "non-container", { &i.position(), i.type() } } } ), pos );
      }
      return r;
   }
   ...
   inline std::size_t erase_minus( const position& pos, concat& l, const key& p, const part& f )
   {
      ...
      throw pegtl::parse_error( format( __FUNCTION__, "array has no last element to delete", { { "array", { &l.p } } } ), pos );
   }
