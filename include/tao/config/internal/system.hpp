...
namespace tao::config::internal
{
   ...
   inline std::string get_env_throws( const position& pos, const std::string& e )
   {
      ...
      throw pegtl::parse_error( format( __FUNCTION__, "environment variable not found", { { "variable", e } } ), pos );
   }
   ...
   inline std::string shell_popen_throws( const position& pos, const std::string& c )
   {
      ...
      if( !f ) {
         throw pegtl::parse_error( format( __FUNCTION__, "popen failed", { { "command", c }, { "errno", errno } } ), pos );
      }
      std::string r;
      char b[ 4096 ];
