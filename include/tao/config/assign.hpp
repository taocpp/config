...
namespace tao::config
{
   ...
   json::basic_value< Traits >& assign( json::basic_value< Traits >& v, const key& k )
   {
      ...
      switch( k[ 0 ].type() ) {
         ...
         case part::star:
            throw std::runtime_error( internal::format( __FUNCTION__, "attempt to assign to star", { &v.key, &v.position } ) );
         case part::minus:
            throw std::runtime_error( internal::format( __FUNCTION__, "attempt to assign to minus", { &v.key, &v.position } ) );
      }
      assert( false );
   }
