...
Note that `json` is frequently combined with `read` as in `foo = (json (read "filename.json"))` and with the file contents being automatically validated as UTF-8 before invoking the JSON parser.
